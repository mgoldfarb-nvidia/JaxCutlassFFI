...
def gemm_cd_major(d: str):
    """Returns order for C/D tensor major mode."""
    return {"n": "lmn", "m": "lnm"}[d]


def gemm_cd_mode(d: str) -> tuple[int, ...]:
    """Returns mode for C/D tensor major mode."""
    return reorder_modes(gemm_cd_major(d), "mnl")
...
def gemm_cd_shape(l, m, n, major) -> tuple[int, ...]:
    """Returns shape for C/D tensor given major mode."""
    assert major in ("m", "n")
...
def create_cd_tensor(
    ...
    *,
    ...
    shape = gemm_cd_shape(l, m, n, major)
    tensor = create_tensor(
        shape,
        dtype,
        key,
...
def gemm_reference_einsum(
    a, b, acc_dtype, cd_dtype, a_major, b_major, cd_major, sf_a=None, sf_b=None
):
    a_idx = gemm_a_major(a_major)
    b_idx = gemm_b_major(b_major)
    cd_idx = gemm_cd_major(cd_major)
    spec = f"{a_idx},{b_idx}->{cd_idx}"
