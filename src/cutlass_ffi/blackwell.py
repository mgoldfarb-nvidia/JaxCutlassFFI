...
def blackwell_group_gemm_block_scaled(a, b, asf, bsf, group_sizes):
    ...
    problem_shapes = jnp.concatenate(
        [group_sizes.reshape(-1, 1), n_sizes.reshape(-1, 1), k_sizes.reshape(-1, 1)], axis=-1
    )
    ...
    workspace_size, cutlass_workspace_size = 16*1024*num_groups, 113664*8*num_groups
    call = jax.ffi.ffi_call(
        "blackwell_group_gemm_block_scaled",
        result_shape_dtypes=(
